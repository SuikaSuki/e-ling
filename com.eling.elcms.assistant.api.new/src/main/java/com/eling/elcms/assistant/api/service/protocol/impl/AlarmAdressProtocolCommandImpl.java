...
@Service
public class AlarmAdressProtocolCommandImpl  implements IProtocolCommand,BeanFactoryAware{
	...
	private static final Logger logger = LoggerFactory.getLogger(AlarmAdressProtocolCommandImpl.class);
	...
	public String process(String para, String imei) {
		...
		coordinate.setRadius(Integer.valueOf(StringUtils.isNoneBlank(location.getRadius()) ? location.getRadius() : "0"));
		//推送消息至APP
		beanFactory.getBeansOfType(IAssistantEventHandler.class)
		.values()
		.stream()
		.forEach(a->a.alarmSos(coordinate, imei));
		//assistantManager.getAssistantElcmsSender().alarmSos(null);
		return result ;
	}
	@Override
