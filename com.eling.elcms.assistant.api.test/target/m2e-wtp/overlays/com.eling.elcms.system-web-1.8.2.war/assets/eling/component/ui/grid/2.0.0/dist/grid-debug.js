 ...
 */
define("eling/component/ui/grid/2.0.0/dist/grid-debug", [ "jquery/jquery-plugins/pagination/pagination", "eling/component/core/uicomponent/2.0.0/src/uicomponent", "base", "tools", "handlebars", "theme", "templatable", "bootstrap", "gallery/handlebars/2.0.0/handlebars-seajs", "eling/component/utils/tools/tools", "./grid.tpl", "eling/component/utils/ajaxwrapper", "dialog", "store", "./grid.css" ], function(require, exports, module) {
    ...
    var Grid = UIComponent.extend({
        ...
        _setData: function(data, isInitPageBar) {
            //model.data存放的只是当前页的数据
            var model = this.get("model");
            model.data = data;
            //设置grid总页数和总记录数
            var totalData = this.get("data");
            var totalText = "共" + (model.isInitPageBar ? Math.ceil(totalData.length / model.items_per_page) + "页，" + totalData.length + "条记录" : totalData.length + "条记录");
        ...
        getIndex: function(el) {
            ...
            if ($(el).is("tr")) {
                return this.$(el).prevAll().size() + currentPage * model.items_per_page;
            } else {
                return this.$(el).parents("tr").prevAll().size() + currentPage * model.items_per_page;
            }
        },
