define(function(require,exports,module){
	...
	return {
		...
		gridformat_button : function(params,value,row,model){
			...
			for(var i in params){
				...
				if(typeof show === "boolean" && show === true){
					ret+="<button style='margin-left:5px;' class='J-grid-"+model.id+"-"+(params[i].id || params[i].key) + " btn btn-xs btn-danger'>";
		...
		gridformat_link : function(params,value,row,model){
			return "<a class='J-grid-"+model.id+"-"+(params.id || params.key)+"' style='color:#f34541;' href='javascript:void(0);'>"+value+"</a>";
		},
