	...
	var GridFormat={
		...
		button:function(params,value,row,model){
			...
			for(var i=0;i<params.length;i++){
				...
				if(show){
					ret+="<a style='margin-left:5px;color:white;background:"+model.theme+"' class='J-"+(params[i].id || params[i].key)+" btn btn-xs "+(params[i].show===false ? "hidden" : "")+"' href='javascript:void(0);'>";
					if(params[i].text){
						ret+=params[i].text;
					}else{
		...
		detail:function(param,value,row,model){
			return "<a class='J-"+((param && (param[0].key || param[0].id)) ?  (param[0].key || param[0].id) : "detail")+"' style='color:"+model.theme+";' href='javascript:void(0);'>"+value+"</a>";
		},
