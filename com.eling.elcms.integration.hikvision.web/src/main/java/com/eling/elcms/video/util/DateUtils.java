...
public class DateUtils {
	...
	public static final String[] pattern = new String[] { "yyyyMMdd", "yyyy-MM-dd", "yyyy/MM/dd", "MM月dd日", "yyyy年",
			"yyyy年MM月", "yyyy年MM月dd日", "yyyyMMddHHmmss", "yyyy-MM-dd HH:mm:ss", "yyyy/MM/dd HH:mm:ss", "公元前y年",
			"公元前yy年", "公元前yyy年", "公元y年", "公元yy年", "公元yyy年", "yyyy年MM月dd日 HH:mm", "yyyy年MM月dd日HH:mm", "yyyy-MM-dd HH:mm",
			"yyyy年MM月dd日 HH:mm:ss", "yyyy-MM-dd HH:mm:ss.sss" };
	...
	public static String getDateTimeString(long mill, int format) {
		...
		case 1: // '\001'
			ret = year + "-" + (month >= 10 ? "" + month : "0" + month) + "-" + (day >= 10 ? "" + day : "0" + day);
			break;

		case 2: // '\002'
			ret = year + "-" + (month >= 10 ? "" + month : "0" + month) + "-" + (day >= 10 ? "" + day : "0" + day) + " "
					+ (hour >= 10 ? "" + hour : "0" + hour) + ":" + (mm >= 10 ? "" + mm : "0" + mm) + ":"
					+ (ss >= 10 ? "" + ss : "0" + ss);
			break;
		...
		case 4: // '\004'
			ret = (year + "").substring(2) + (month >= 10 ? "" + month : "0" + month)
					+ (day >= 10 ? "" + day : "0" + day);
			break;
		...
		case 6: // '\006'
			ret = year + "" + "\u5E74" + (month >= 10 ? "" + month : "0" + month) + "\u6708"
					+ (day >= 10 ? "" + day : "0" + day) + "\u65E5";
			break;
		}
		return ret;
	...
	public static int substract(String biger_date_time, String smaller_date_time) {
		return (int) ((getMillisecond(formatDateTimeString(biger_date_time, 1))
				- getMillisecond(formatDateTimeString(smaller_date_time, 1))) / 0x5265c00L);
	}

	public static int substract3(String biger_date_time, String smaller_date_time) {
		return (int) ((getMillisecond(formatDateTimeString(biger_date_time, 1))
				- getMillisecond(formatDateTimeString(smaller_date_time, 1))) / 0x5265c00L) + 1;
	}
	...
	public static BigDecimal substract2(String biger_date_time, String smaller_date_time) {
		...
		BigDecimal mth = (new BigDecimal(termArr[0]))
				.add((new BigDecimal(termArr[1])).divide(new BigDecimal(30D), 1, 4));
		return mth;
	}
	...
	public static String add(String date_time_str, int days, int format) {
		if (format == 1)
			...
			return getDateTimeString(getMillisecond(date_time_str) + (long) (days * 0x5265c00), format);
	}
	...
	public static int subtract(String strEndDate, String strStartDate, String format) {
		DateFormat df = getDateFormat(format);
	...
	public static boolean isInDatePeriod(String someDate, String begin_date, String end_date) {
		return substract(someDate, begin_date) >= 0 && substract(someDate, end_date) <= 0;
	}
	 ...
	 * 取得两个时间相差多少秒.
	 */
	public static long getSubstractSecond(String strEndDate, String strStartDate) {
		long msEndDate = getMillisecond(strEndDate);
		long msStartDate = getMillisecond(strStartDate);
		long temp = msEndDate - msStartDate;
		long result = (long) temp / (1000);
		return result;
	}
	...
	public static String logFormatTransform(String time) {
		...
		if (time != null && !time.trim().equals("")) {
			newStr = time.substring(0, 4) + "-" + time.substring(4, 6) + "-" + time.substring(6, 8);
			if (time.length() > 8)
				newStr = newStr + " " + time.substring(8, 10) + ":" + time.substring(10, 12) + ":"
	...
	public static String getCurrentTimeLogFormat() {
		return getCurrentDateTime().replaceAll("-", "").replaceAll(":", "").replaceAll(" ", "");
	}

	public static String[] getBalanceCycleMon(String bal_year_start, String bal_year_end) {
		int nyear = Integer.parseInt(bal_year_end) - Integer.parseInt(bal_year_start);
		...
		for (int m = 0; m < nyear; m++) {
			for (int n = 1; n <= 12; n++) {
				...
				bal_cycle[seq++] = next_year + next_mon + "01" + next_year + next_mon + getDayOfMonth(next_year, n)
						+ next_year + "\u5E74" + n + "\u6708";
			}
	...
	public static String[] getBalanceCycleQua(String bal_year_start, String bal_year_end) {
		int nyear = Integer.parseInt(bal_year_end) - Integer.parseInt(bal_year_start);
		...
		for (int m = 0; m < nyear; m++) {
			...
			for (int j = 1; n <= 12; j++) {
				...
				bal_cycle[seq++] = next_year + next_mon + "01" + next_year + next_mon_2 + getDayOfMonth(next_year, n)
						+ next_year + "\u5E74" + j + "\u5B63\u5EA6";
				n++;
			}
	...
	public static String convertDate2StringWithoutYear(Date date) throws ParseException {
	...
	public static String orderNum() {
		...
		createTime = String.valueOf(new Timestamp(new Date().getTime()).toString());
		createTime = createTime.replace("-", "").substring(2, 8);
		return "H" + createTime + String.valueOf(new Date().getTime()).substring(5);
	}
